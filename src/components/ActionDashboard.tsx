...
const EmergencyMap = React.lazy(() =>
  import('./EmergencyMap').then(m => ({ default: m.EmergencyMap }))
);

const MAP_CATEGORIES = new Set(['MEDICAL', 'EMERGENCY', 'TRAFFIC']);
    ...
    >
      ...
      <div className="flex flex-col gap-8" role="feed" aria-busy="false">
        {actions.map((action, idx) => {
          ...
          const showMap = MAP_CATEGORIES.has(action.category);
          return (
            <article
              key={idx}
              className="glass-card flex flex-col gap-8 animate-enter"
            ...
            >
              ...
              {/* Google Maps — only for location-relevant categories */}
              {showMap && (
                <Suspense fallback={
                  ...
                  <EmergencyMap
                    ...
                    extractedIntent={action.extractedIntent}
                  />
                </Suspense>
              )}
            </article>
